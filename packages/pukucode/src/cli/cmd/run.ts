...
export const RunCommand = cmd({
  command: "run [message..]",
  describe: "run pukucode with a message",
  builder: (yargs: Argv) => {
    return yargs
      .positional("message", {
  ...
  handler: async (args) => {
    ...
    await bootstrap({ cwd: process.cwd() }, async () => {
      ...
      if (cfg.share === "auto" || Flag.get("PUKUCODE_AUTO_SHARE") || args.share) {
        try {
          await Session.share(session.id)
          UI.println(UI.Style.TEXT_INFO_BOLD + "~  Session shared: " + session.id.slice(-8))
        } catch (error) {
