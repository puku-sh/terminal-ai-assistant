...
export namespace Config {
  ...
  export const state = App.state("config", async (app) => {
    ...
    for (const file of ["pukucode.jsonc", "pukucode.json"]) {
    ...
    // if (Flag.pukucode_CONFIG) {
    //     result = mergeDeep(result, await loadFile(Flag.pukucode_CONFIG))
    //     log.debug("loaded custom config", { path: Flag.pukucode_CONFIG })
    //   }

    for (const [key, value] of Object.entries(auth)) {
      if (value.type === "wellknown") {
        process.env[value.key] = value.token
        const wellknown = await fetch(`${key}/.well-known/pukucode`).then((x) => x.json())
    ...
    const markdownAgents = [
      ...
      ...(await Filesystem.globUp(".pukucode/agent/**/*.md", app.path.cwd, app.path.root)),
    ]
    for (const item of markdownAgents) {
      ...
      const agentFolderPath = item.includes("/.s/agent/")
        ? item.split("/.pukucode/agent/")[1]
        : item.includes("/agent/")
          ? item.split("/agent/")[1]
          : agentName + ".md"
    ...
    const markdownModes = [
      ...
      ...(await Filesystem.globUp(".pukucode/mode/*.md", app.path.cwd, app.path.root)),
    ]
    ...
    const markdownCommands = [
      ...
      ...(await Filesystem.globUp(".pukucode/command/*.md", app.path.cwd, app.path.root)),
    ]
    ...
    result.plugin.push(
      ...[
        ...
        ...(await Filesystem.globUp(".pukucode/plugin/*.{ts,js}", app.path.cwd, app.path.root)),
      ].map((x) => "file://" + x),
    )

    // if (Flag.pukucode_PERMISSION) {
    //   result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.pukucode_PERMISSION))
    // }
  ...
  export const Info = z
    .object({
      ...
      agent: z
        ...
        .catchall(Agent)
        .optional()
        .describe("Agent configuration, see https://pukucode.ai/docs/agent"),
      provider: z
        .record(
  ...
  export const global = lazy(async () => {
    let result: Info = pipe(
      ...
      mergeDeep(await loadFile(path.join(Global.Path.config, "pukucode.json"))),
      mergeDeep(await loadFile(path.join(Global.Path.config, "pukucode.jsonc"))),
    )
    ...
    await import(path.join(Global.Path.config, "config"), {
      ...
      .then(async (mod) => {
        ...
        result["$schema"] = "./pukucode.schema.json"
